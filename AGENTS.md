...
- Auth Google OAuth redirect URIs are not configured for localhost, so authenticated flows (posting, messaging, dashboard) can't be exercised end-to-end in cloud. Verify DB writes via Prisma directly using `lib/prisma.ts`'s Neon adapter.
